// lib/data/food_booths.dart
import '../models/food_booth.dart';
import '../models/dish.dart';

final List<FoodBooth> foodBooths = [
  FoodBooth(
    name: "Miraku Boston",
    image: "assets/foodLogo/miraku_boston.png",
    description: "Zangi, sweets",
    allergy: "Contains gluten",
    boothLocation: "B2",
    genre: "Japanese",
    logoPath: "assets/foodLogo/miraku_boston.png",
    dishImagePath: "assets/food/zangi.jpg",
    isVegan: true,
    payments: ["Cash", "Venmo"],
    allergens: ["Wheat"],
    dishes: [
      Dish(
        name: "Zangi (Hokkaido-style Fried Chicken)",
        description: "Crispy fried chicken pieces marinated in special sauce",
        price: 8.99,
        allergens: ["Wheat", "Soy", "Egg", "Sesame"],
        imagePath: "assets/food/zangi.jpg",
        isVegan: false,
      ),
      Dish(
        name: "Cheescake",
        description: "Soft, creamy, and fluffy dessert, very tasty",
        price: 5.99,
        allergens: ["Wheat", "Egg", "Milk"],
        imagePath: "assets/food/cheescake.jpg",
        isVegan: true,
      ),
    ],
  ),
  FoodBooth(
    name: "Cafe Itadaki",
    image: "assets/foodLogo/cafe_itadaki.jpg",
    description: "Takoyaki, fried chicken, snacks, Ramune",
    allergy: "Contains wheat, soy",
    boothLocation: "D1",
    genre: "Japanese",
    logoPath: "assets/foodLogo/cafe_itadaki.jpg",
    dishImagePath: "assets/food/itadaki_takoyaki.jpg",
    isVegan: false,
    payments: ["Cash"],
    allergens: ["Wheat", "Soy"],
    dishes: [
      Dish(
        name: "Takoyaki",
        description: "Octopus balls topped with sauce, mayo, and bonito flakes",
        price: 7.99,
        allergens: ["Wheat", "Soy", "Fish", "Egg"],
        imagePath: "assets/food/itadaki_takoyaki.png",
        isVegan: false,
      ),
      Dish(
        name: "Karaage",
        description: "Japanese-style fried chicken",
        price: 8.99,
        allergens: ["Wheat", "Soy"],
        imagePath: "assets/karaage.png",
        isVegan: false,
      ),
      Dish(
        name: "Ramune Soda",
        description: "Japanese marble soda in various flavors",
        price: 3.99,
        allergens: [],
        imagePath: "assets/ramune.png",
        isVegan: true,
      ),
    ],
  ),
  FoodBooth(
    name: "Tori Jiro",
    image: "assets/food/tori_jori.jpg",
    description: "Yakitori, grilled skewers, dessert, drinks",
    allergy: "Contains soy",
    boothLocation: "D8",
    genre: "Japanese",
    logoPath: "assets/tori_jiro_logo.jpg",
    dishImagePath: "assets/food/tori_jori.jpg",
    isVegan: false,
    payments: ["Cash"],
    allergens: ["Soy"],
    dishes: [
      Dish(
        name: "Chicken Yakitori",
        description: "Grilled chicken skewers with tare sauce",
        price: 6.99,
        allergens: ["Soy"],
        imagePath: "assets/yakitori_chicken.png",
        isVegan: false,
      ),
      Dish(
        name: "Vegetable Skewers",
        description: "Grilled seasonal vegetables with salt",
        price: 5.99,
        allergens: [],
        imagePath: "assets/veggie_skewers.png",
        isVegan: true,
      ),
      Dish(
        name: "Dorayaki",
        description: "Japanese pancake with sweet red bean filling",
        price: 4.99,
        allergens: ["Wheat", "Egg"],
        imagePath: "assets/dorayaki_dessert.png",
        isVegan: false,
      ),
    ],
  ),
  FoodBooth(
    name: "Tokyo Teriyaki Chicken",
    image: "assets/tokyo_teriyaki.png",
    description: "Teriyaki Chicken Bowl, Matcha Dessert and Boba Tea",
    allergy: "Contains soy, dairy",
    boothLocation: "D2",
    genre: "Japanese",
    logoPath: "assets/tokyo_teriyaki.png",
    dishImagePath: "assets/teriyaki_dish.png",
    isVegan: false,
    payments: ["Cash"],
    allergens: ["Soy", "Milk"],
    dishes: [
      Dish(
        name: "Teriyaki Chicken Bowl",
        description: "Grilled chicken with teriyaki sauce over rice",
        price: 10.99,
        allergens: ["Soy"],
        imagePath: "assets/teriyaki_dish.png",
        isVegan: false,
      ),
      Dish(
        name: "Matcha Ice Cream",
        description: "Green tea flavored ice cream",
        price: 4.99,
        allergens: ["Milk"],
        imagePath: "assets/matcha_ice_cream.png",
        isVegan: false,
      ),
      Dish(
        name: "Boba Milk Tea",
        description: "Sweet milk tea with tapioca pearls",
        price: 5.99,
        allergens: ["Milk"],
        imagePath: "assets/boba_tea.png",
        isVegan: false,
      ),
    ],
  ),
  
  FoodBooth(
    name: "AKITA RICE SMOOTHIE",
    image: "assets/akita_smoothie.png",
    description: "Salmon teriyaki onigiri, Kombu tsukudani onigiri, Matcha smoothie",
    allergy: "Contains fish, soy",
    boothLocation: "A2-1",
    genre: "Japanese",
    logoPath: "assets/akita_logo.png",
    dishImagePath: "assets/smoothie_dish.png",
    isVegan: false,
    payments: ["Cash"],
    allergens: ["Fish", "Soy"],
    dishes: [
      Dish(
        name: "Salmon Teriyaki Onigiri",
        description: "Rice ball filled with teriyaki-flavored salmon",
        price: 4.99,
        allergens: ["Fish", "Soy"],
        imagePath: "assets/salmon_onigiri.png",
        isVegan: false,
      ),
      Dish(
        name: "Kombu Tsukudani Onigiri",
        description: "Rice ball with seasoned seaweed filling",
        price: 4.50,
        allergens: ["Soy"],
        imagePath: "assets/kombu_onigiri.png",
        isVegan: true,
      ),
      Dish(
        name: "Matcha Smoothie",
        description: "Creamy matcha green tea smoothie",
        price: 6.99,
        allergens: ["Milk"],
        imagePath: "assets/smoothie_dish.png",
        isVegan: false,
      ),
    ],
  ),
  FoodBooth(
    name: "Beard Papa's",
    image: "assets/beard_papas.png",
    description: "Cream puffs and boba tea",
    allergy: "Contains dairy, wheat",
    boothLocation: "E6",
    genre: "Bakery",
    logoPath: "assets/beard_papas.png",
    dishImagePath: "assets/creampuff_dish.png",
    isVegan: false,
    payments: ["Cash"],
    allergens: ["Milk", "Wheat"],
    dishes: [
      Dish(
        name: "Vanilla Cream Puff",
        description: "Crispy shell filled with vanilla custard cream",
        price: 4.99,
        allergens: ["Milk", "Wheat", "Egg"],
        imagePath: "assets/vanilla_cream_puff.png",
        isVegan: false,
      ),
      Dish(
        name: "Matcha Cream Puff",
        description: "Crispy shell filled with matcha green tea custard cream",
        price: 5.49,
        allergens: ["Milk", "Wheat", "Egg"],
        imagePath: "assets/matcha_cream_puff.png",
        isVegan: false,
      ),
      Dish(
        name: "Milk Tea",
        description: "Classic milk tea with tapioca pearls",
        price: 5.99,
        allergens: ["Milk"],
        imagePath: "assets/milk_tea.png",
        isVegan: false,
      ),
    ],
  ),
  FoodBooth(
    name: "Wakazo Gyoza",
    image: "assets/food/wakazo_gyoza.jpg",
    description: "Gyoza, water, soft drinks",
    allergy: "Contains wheat",
    boothLocation: "D7",
    genre: "Japanese",
    logoPath: "assets/miraku_boston.png",
    dishImagePath: "assets/food/wakazo_gyoza.jpg",
    isVegan: false,
    payments: ["Cash", "Venmo", "Zelle", "PayPal"],
    allergens: ["Wheat", "Soy"],
    dishes: [
      Dish(
        name: "Pan-fried Gyoza (6 pcs)",
        description: "Japanese dumplings filled with pork and vegetables",
        price: 7.99,
        allergens: ["Wheat", "Soy"],
        imagePath: "assets/food/wakazo_gyoza.jpg",
        isVegan: false,
      ),
      Dish(
        name: "Vegetable Gyoza (6 pcs)",
        description: "Dumplings filled with cabbage, carrots, and mushrooms",
        price: 7.49,
        allergens: ["Wheat", "Soy"],
        imagePath: "assets/veggie_gyoza.png",
        isVegan: true,
      ),
      Dish(
        name: "Bottled Water",
        description: "500ml bottled water",
        price: 2.00,
        allergens: [],
        imagePath: "assets/water_bottle.png",
        isVegan: true,
      ),
    ],
  ),
  FoodBooth(
    name: "Ichiban Kimchi",
    image: "assets/ichiban_kimchi.png",
    description: "Kimchi, reference:",
    allergy: "Contains soy",
    boothLocation: "E4",
    genre: "Korean",
    logoPath: "assets/ichiban_kimchi.png",
    dishImagePath: "assets/kimchi_dish.png",
    isVegan: true,
    payments: ["Cash"],
    allergens: ["Soy"],
    dishes: [
      Dish(
        name: "Napa Cabbage Kimchi",
        description: "Traditional fermented napa cabbage with Korean spices",
        price: 8.99,
        allergens: ["Soy"],
        imagePath: "assets/kimchi_dish.png",
        isVegan: true,
      ),
      Dish(
        name: "Cucumber Kimchi",
        description: "Fresh cucumber kimchi with garlic and chili",
        price: 7.99,
        allergens: ["Soy"],
        imagePath: "assets/cucumber_kimchi.png",
        isVegan: true,
      ),
    ],
  ),
  FoodBooth(
    name: "JAPAN ONIGIRI Akitaya",
    image: "assets/akitaya_logo.png",
    description: "Rice ball (onigiri)",
    allergy: "Contains soy, seaweed",
    boothLocation: "A2-2",
    genre: "Japanese",
    logoPath: "assets/akitaya_logo.png",
    dishImagePath: "assets/onigiri_dish.png",
    isVegan: true,
    payments: ["Cash"],
    allergens: ["Soy"],
    dishes: [
      Dish(
        name: "Umeboshi Onigiri",
        description: "Rice ball with pickled plum filling",
        price: 3.99,
        allergens: [],
        imagePath: "assets/umeboshi_onigiri.png",
        isVegan: true,
      ),
      Dish(
        name: "Seaweed Onigiri",
        description: "Rice ball filled with seasoned seaweed",
        price: 3.99,
        allergens: ["Soy"],
        imagePath: "assets/onigiri_dish.png",
        isVegan: true,
      ),
      Dish(
        name: "Tuna Mayo Onigiri",
        description: "Rice ball with tuna and mayonnaise filling",
        price: 4.50,
        allergens: ["Fish", "Egg", "Soy"],
        imagePath: "assets/tuna_onigiri.png",
        isVegan: false,
      ),
    ],
  ),
  

  FoodBooth(
    name: "IBARAKI's Yaki-Onigiri",
    image: "assets/yakionigir.png",
    description: "Yaki-Onigiri (Soy Sauce), Yaki-Onigiri (Miso)",
    allergy: "Contains Soy, Wheat",
    boothLocation: "D5-2",
    genre: "Japanese",
    logoPath: "assets/yakionigiri.png",
    dishImagePath: "assets/17. Yaki Onigiri.jpg",
    isVegan: true,
    payments: ["Cash", "Venmo"],
    allergens: ["Soy", "Wheat"],
    dishes: [
      Dish(
        name: "Yaki-Onigiri (Soy Sauce)",
        description: "Grilled rice ball brushed with soy sauce",
        price: 4.99,
        allergens: ["Soy", "Wheat"],
        imagePath: "assets/17. Yaki Onigiri.jpg",
        isVegan: true,
      ),
      Dish(
        name: "Yaki-Onigiri (Miso)",
        description: "Grilled rice ball brushed with miso sauce",
        price: 4.99,
        allergens: ["Soy", "Wheat"],
        imagePath: "assets/miso_yaki_onigiri.png",
        isVegan: true,
      ),
    ],
  ),
  FoodBooth(
    name: "Lady M Cake Boutique",
    image: "assets/foodLogo/Lady_M.jpg",
    description: "Signature Mille Crepes, Green Tea Mille Crepes, Pistachio Mille Crepes",
    allergy: "Contains Egg, Wheat, Milk, Tree Nuts",
    boothLocation: "E-8",
    genre: "Bakery",
    logoPath: "assets/foodLogo/Lady_M.jpg",
    dishImagePath: "assets/4. Lady M.png",
    isVegan: true,
    payments: ["Cash", "Credit"],
    allergens: ["Egg", "Wheat", "Milk", "Tree Nuts"],
    dishes: [
      Dish(
        name: "Signature Mille Crepes",
        description: "Twenty layers of thin handmade crepes with light pastry cream",
        price: 9.50,
        allergens: ["Egg", "Wheat", "Milk"],
        imagePath: "assets/signature_mille_crepes.jpg",
        isVegan: true,
      ),
      Dish(
        name: "Green Tea Mille Crepes",
        description: "Twenty layers of thin handmade crepes with matcha green tea cream",
        price: 9.99,
        allergens: ["Egg", "Wheat", "Milk", "Tree Nut"],
        imagePath: "assets/food/green_tea_cheescake.png",
        isVegan: true,
      ),
      Dish(
        name: "Pistachio Mille Crepes",
        description: "Twenty layers of thin handmade crepes with pistachio cream",
        price: 10.50,
        allergens: ["Egg", "Wheat", "Milk", "Tree Nuts"],
        imagePath: "assets/pistachio_mille_crepe.png",
        isVegan: true,
      ),
    ],
  ),
  
  // Yawaragi International (和インターナショナル)
FoodBooth(
  name: "Yawaragi International",
  image: "assets/food/japanese_sweets.jpg",
  description: "Traditional Japanese sweets and green tea",
  allergy: "N/A",
  boothLocation: "TBD",
  genre: "Japanese Confectionery",
  logoPath: "assets/yawaragi_international.png",
  dishImagePath: "assets/food/japanese_sweets.jpg",
  isVegan: true, // All items marked Yes for vegetarian
  payments: ["Cash", "Venmo"],
  allergens: [],
  dishes: [
    Dish(
      name: "Nerikiri",
      description: "Traditional Japanese sweet",
      price: 5.00,
      allergens: [],
      imagePath: "assets/food/nerikiri.jpg",
      isVegan: true,
    ),
    Dish(
      name: "Higashi",
      description: "Dry Japanese sweet",
      price: 4.50,
      allergens: [],
      imagePath: "assets/food/higashi.jpg",
      isVegan: true,
    ),
    Dish(
      name: "Konpeito",
      description: "Japanese sugar candy",
      price: 3.50,
      allergens: [],
      imagePath: "assets/food/konpeito.jpg",
      isVegan: true,
    ),
    Dish(
      name: "Green Tea",
      description: "Traditional Japanese green tea",
      price: 3.00,
      allergens: [],
      imagePath: "assets/food/green_tea.jpg",
      isVegan: true,
    ),
  ],
),

// Yume wo Katare
FoodBooth(
  name: "Yume wo Katare",
  image: "assets/food/ramen.jpg",
  description: "Japanese ramen",
  allergy: "Contains wheat, fish",
  boothLocation: "TBD",
  genre: "Ramen",
  logoPath: "assets/yume_wo_katare.png",
  dishImagePath: "assets/food/ramen.jpg",
  isVegan: false,
  payments: ["Cash", "Venmo"],
  allergens: ["Wheat", "Fish"],
  dishes: [
    Dish(
      name: "Ramen",
      description: "Traditional Japanese ramen",
      price: 12.00,
      allergens: ["Wheat", "Fish"],
      imagePath: "assets/food/ramen.jpg",
      isVegan: false,
    ),
  ],
),

// RamenNIKKOU (ラーメンにっこう)
FoodBooth(
  name: "RamenNIKKOU",
  image: "assets/food/ramen.jpg",
  description: "Kottreri Paitan Ramen",
  allergy: "Contains wheat, soy, sesame",
  boothLocation: "TBD",
  genre: "Ramen",
  logoPath: "assets/ramen_nikkou.png",
  dishImagePath: "assets/food/ramen.jpg",
  isVegan: false,
  payments: ["Cash", "Venmo"],
  allergens: ["Wheat", "Soy", "Sesame"],
  dishes: [
    Dish(
      name: "Kottreri Paitan Ramen",
      description: "Rich and creamy chicken ramen",
      price: 13.00,
      allergens: ["Wheat", "Soy", "Sesame"],
      imagePath: "assets/food/paitan_ramen.jpg",
      isVegan: false,
    ),
  ],
),

// Beard Papa's
FoodBooth(
  name: "Beard Papa's",
  image: "assets/food/cream_puff.jpg",
  description: "Cream puffs and milk tea",
  allergy: "Contains egg, wheat, milk",
  boothLocation: "TBD",
  genre: "Dessert",
  logoPath: "assets/beard_papas.png",
  dishImagePath: "assets/food/cream_puff.jpg",
  isVegan: false,
  payments: ["Cash", "Zelle", "Card"],
  allergens: ["Egg", "Wheat", "Milk"],
  dishes: [
    Dish(
      name: "Cream Puff",
      description: "Fresh cream puff pastry",
      price: 4.50,
      allergens: ["Egg", "Wheat", "Milk"],
      imagePath: "assets/food/cream_puff.jpg",
      isVegan: false,
    ),
    Dish(
      name: "Milk Tea",
      description: "Sweetened milk tea",
      price: 4.00,
      allergens: ["Milk"],
      imagePath: "assets/food/milk_tea.jpg",
      isVegan: true, // Assuming milk can be substituted
    ),
  ],
),

// Umai Beef Bowl
FoodBooth(
  name: "Umai Beef Bowl",
  image: "assets/food/beef_bowl.jpg",
  description: "Japanese beef rice bowls",
  allergy: "Contains soy",
  boothLocation: "TBD",
  genre: "Japanese",
  logoPath: "assets/umai_beef_bowl.png",
  dishImagePath: "assets/food/beef_bowl.jpg",
  isVegan: false,
  payments: ["Cash", "Zelle", "Venmo"],
  allergens: ["Soy"],
  dishes: [
    Dish(
      name: "Beef Bowl",
      description: "Japanese style beef rice bowl",
      price: 10.00,
      allergens: ["Soy"],
      imagePath: "assets/food/beef_bowl.jpg",
      isVegan: false,
    ),
  ],
),

// Nagomi Bento (なごみ弁当)
FoodBooth(
  name: "Nagomi Bento",
  image: "assets/food/bento.jpg",
  description: "Japanese bento boxes and drinks",
  allergy: "Contains egg, soy",
  boothLocation: "TBD",
  genre: "Japanese",
  logoPath: "assets/nagomi_bento.png",
  dishImagePath: "assets/food/bento.jpg",
  isVegan: false, // Mixed vegan options
  payments: ["Cash", "Credit Card"],
  allergens: ["Egg", "Soy"],
  dishes: [
    Dish(
      name: "Nagomi Gyudon",
      description: "Beef rice bowl",
      price: 11.00,
      allergens: ["Egg", "Soy"],
      imagePath: "assets/food/gyudon.jpg",
      isVegan: false,
    ),
    Dish(
      name: "Matcha Lemonade",
      description: "Refreshing matcha lemonade",
      price: 4.50,
      allergens: [],
      imagePath: "assets/food/matcha_lemonade.jpg",
      isVegan: true,
    ),
    Dish(
      name: "Barley Tea",
      description: "Traditional Japanese barley tea",
      price: 3.50,
      allergens: [],
      imagePath: "assets/food/barley_tea.jpg",
      isVegan: true,
    ),
  ],
),

// Hokkaido Ramen Santouka (北海道らーめん山頭火)
FoodBooth(
  name: "Hokkaido Ramen Santouka",
  image: "assets/food/ramen.jpg",
  description: "Hokkaido style ramen and snacks",
  allergy: "Contains egg, wheat, milk, soy, fish, sesame, peanut, tree nuts, shellfish",
  boothLocation: "TBD",
  genre: "Ramen",
  logoPath: "assets/santouka.png",
  dishImagePath: "assets/food/ramen.jpg",
  isVegan: false,
  payments: ["Credit Card"],
  allergens: ["Egg", "Wheat", "Milk", "Soy", "Fish", "Sesame", "Peanut", "Tree Nuts", "Shellfish"],
  dishes: [
    Dish(
      name: "Ramen",
      description: "Hokkaido style ramen",
      price: 14.00,
      allergens: ["Egg", "Wheat", "Milk", "Soy", "Fish", "Sesame"],
      imagePath: "assets/food/ramen.jpg",
      isVegan: false,
    ),
    Dish(
      name: "Potato Stick",
      description: "Crispy potato snack",
      price: 4.00,
      allergens: ["Egg", "Wheat", "Peanut", "Milk", "Soy", "Tree Nuts", "Fish", "Shellfish", "Sesame"],
      imagePath: "assets/food/potato_stick.jpg",
      isVegan: false,
    ),
  ],
),

// Kevin's Bread (けびんのパン)
FoodBooth(
  name: "Kevin's Bread",
  image: "assets/food/japanese_bread.jpg",
  description: "Japanese style breads",
  allergy: "Contains egg, wheat, milk",
  boothLocation: "TBD",
  genre: "Bakery",
  logoPath: "assets/kevins_bread.png",
  dishImagePath: "assets/food/japanese_bread.jpg",
  isVegan: false,
  payments: ["Cash", "Venmo"],
  allergens: ["Egg", "Wheat", "Milk"],
  dishes: [
    Dish(
      name: "Curry Pan",
      description: "Japanese curry bread",
      price: 5.50,
      allergens: ["Egg", "Wheat", "Milk"],
      imagePath: "assets/food/curry_pan.jpg",
      isVegan: false,
    ),
    Dish(
      name: "Sakura An Pan",
      description: "Sweet bread with cherry blossom filling",
      price: 5.00,
      allergens: ["Egg", "Wheat", "Milk"],
      imagePath: "assets/food/sakura_pan.jpg",
      isVegan: false,
    ),
  ],
),

// IBARAKI's Yaki-Onigiri (茨城の焼きおにぎり)
FoodBooth(
  name: "IBARAKI's Yaki-Onigiri",
  image: "assets/food/yaki_onigiri.jpg",
  description: "Grilled rice balls",
  allergy: "Contains soy, wheat",
  boothLocation: "TBD",
  genre: "Japanese",
  logoPath: "assets/ibaraki_onigiri.png",
  dishImagePath: "assets/food/yaki_onigiri.jpg",
  isVegan: true, // All items marked Yes for vegetarian
  payments: ["Cash", "Venmo"],
  allergens: ["Soy", "Wheat"],
  dishes: [
    Dish(
      name: "Yaki-Onigiri (Soy Sauce)",
      description: "Grilled rice ball with soy sauce flavor",
      price: 4.50,
      allergens: ["Soy"],
      imagePath: "assets/food/yaki_onigiri.jpg",
      isVegan: true,
    ),
    Dish(
      name: "Yaki-Onigiri (Miso)",
      description: "Grilled rice ball with miso flavor",
      price: 4.50,
      allergens: ["Wheat", "Soy"],
      imagePath: "assets/food/miso_onigiri.jpg",
      isVegan: true,
    ),
  ],
),


// Sakabayashi Sushi Tavern (酒林)
FoodBooth(
  name: "Sakabayashi Sushi Tavern",
  image: "assets/food/sushi_bento.jpg",
  description: "Japanese bento boxes and snacks",
  allergy: "Contains egg, fish",
  boothLocation: "TBD",
  genre: "Japanese",
  logoPath: "assets/sakabayashi.png",
  dishImagePath: "assets/food/sushi_bento.jpg",
  isVegan: false,
  payments: ["Cash", "Credit Card"],
  allergens: ["Egg", "Fish"],
  dishes: [
    Dish(
      name: "Karaage and Salmon Bento",
      description: "Fried chicken and salmon bento box",
      price: 12.50,
      allergens: ["Egg", "Fish"],
      imagePath: "assets/food/karaage_bento.jpg",
      isVegan: false,
    ),
    Dish(
      name: "Salmon Onigiri",
      description: "Rice ball with salmon filling",
      price: 5.00,
      allergens: ["Fish"],
      imagePath: "assets/food/salmon_onigiri.jpg",
      isVegan: false,
    ),
    Dish(
      name: "Takoyaki",
      description: "Octopus balls",
      price: 6.50,
      allergens: ["Egg"],
      imagePath: "assets/food/takoyaki.jpg",
      isVegan: false,
    ),
  ],
),

// Bosso Ramen Tavern
FoodBooth(
  name: "Bosso Ramen Tavern",
  image: "assets/food/chicken_teriyaki.jpg",
  description: "Japanese chicken dishes",
  allergy: "Contains wheat, soy",
  boothLocation: "TBD",
  genre: "Japanese",
  logoPath: "assets/bosso_ramen.png",
  dishImagePath: "assets/food/chicken_teriyaki.jpg",
  isVegan: false,
  payments: ["Cash", "Venmo"],
  allergens: ["Wheat", "Soy"],
  dishes: [
    Dish(
      name: "Chicken Teriyaki",
      description: "Grilled chicken with teriyaki sauce",
      price: 10.50,
      allergens: ["Wheat", "Soy"],
      imagePath: "assets/food/chicken_teriyaki.jpg",
      isVegan: false,
    ),
  ],
),

// JAPAN ONIGIRI AKITAYA (おにぎり 秋田屋)
FoodBooth(
  name: "JAPAN ONIGIRI AKITAYA",
  image: "assets/food/onigiri.jpg",
  description: "Japanese rice balls",
  allergy: "Contains egg, soy, fish, wheat, milk, sesame",
  boothLocation: "TBD",
  genre: "Japanese",
  logoPath: "assets/akitaya.png",
  dishImagePath: "assets/food/onigiri.jpg",
  isVegan: false, // Mixed options
  payments: ["Cash", "Venmo"],
  allergens: ["Egg", "Soy", "Fish", "Wheat", "Milk", "Sesame"],
  dishes: [
    Dish(
      name: "Onigiri Ume",
      description: "Rice ball with pickled plum",
      price: 4.00,
      allergens: [],
      imagePath: "assets/food/ume_onigiri.jpg",
      isVegan: true,
    ),
    Dish(
      name: "Onigiri Spicy Tuna",
      description: "Rice ball with spicy tuna",
      price: 5.00,
      allergens: ["Egg", "Soy", "Fish"],
      imagePath: "assets/food/tuna_onigiri.jpg",
      isVegan: false,
    ),
    Dish(
      name: "Onigiri Tuna",
      description: "Rice ball with tuna",
      price: 5.00,
      allergens: ["Egg", "Soy", "Fish"],
      imagePath: "assets/food/tuna_onigiri.jpg",
      isVegan: false,
    ),
    Dish(
      name: "Furikake Egg",
      description: "Rice with egg and seasoning",
      price: 5.50,
      allergens: ["Egg", "Wheat", "Milk", "Soy", "Fish", "Sesame"],
      imagePath: "assets/food/furikake_egg.jpg",
      isVegan: false,
    ),
  ],
),

// ROYCE' Chocolate
FoodBooth(
  name: "ROYCE' Chocolate",
  image: "assets/food/chocolate.jpg",
  description: "Premium Japanese chocolates",
  allergy: "Contains milk, soy, tree nuts, wheat",
  boothLocation: "TBD",
  genre: "Dessert",
  logoPath: "assets/royce_chocolate.png",
  dishImagePath: "assets/food/chocolate.jpg",
  isVegan: false, // Contains dairy
  payments: ["Cash", "Debit/Credit Card", "Apple Pay", "Google Pay"],
  allergens: ["Milk", "Soy", "Tree Nuts", "Wheat"],
  dishes: [
    Dish(
      name: "NAMA CHOCOLATE",
      description: "Premium fresh chocolate",
      price: 12.00,
      allergens: ["Milk", "Soy", "Tree Nuts"],
      imagePath: "assets/food/nama_chocolate.jpg",
      isVegan: false,
    ),
    Dish(
      name: "BAR CHOCOLATE",
      description: "Chocolate bar",
      price: 8.00,
      allergens: ["Wheat", "Milk", "Soy", "Tree Nuts"],
      imagePath: "assets/food/chocolate_bar.jpg",
      isVegan: false,
    ),
    Dish(
      name: "PURE CHOCOLATE/PRAFEUILLE CHOCOLAT",
      description: "Pure chocolate",
      price: 10.00,
      allergens: ["Milk", "Soy", "Tree Nuts"],
      imagePath: "assets/food/pure_chocolate.jpg",
      isVegan: false,
    ),
    Dish(
      name: "CHOCOLATE WAFERS",
      description: "Chocolate coated wafers",
      price: 7.50,
      allergens: ["Wheat", "Milk", "Soy", "Tree Nuts"],
      imagePath: "assets/food/chocolate_wafers.jpg",
      isVegan: false,
    ),
    Dish(
      name: "MATCHA ALMOND",
      description: "Matcha flavored almond chocolate",
      price: 9.00,
      allergens: ["Tree Nuts", "Soy", "Milk"],
      imagePath: "assets/food/matcha_almond.jpg",
      isVegan: false,
    ),
    Dish(
      name: "POTATOCHIP CHOCOLATE",
      description: "Chocolate covered potato chips",
      price: 8.50,
      allergens: ["Soy", "Milk"],
      imagePath: "assets/food/potatochip_chocolate.jpg",
      isVegan: false,
    ),
  ],
),

// PAGU
FoodBooth(
  name: "PAGU",
  image: "assets/food/japanese_food.jpg",
  description: "Japanese rice balls and noodles",
  allergy: "Contains soy, fish, sesame, wheat",
  boothLocation: "TBD",
  genre: "Japanese",
  logoPath: "assets/pagu.png",
  dishImagePath: "assets/food/japanese_food.jpg",
  isVegan: false, // Mixed options
  payments: ["Cash", "Venmo"],
  allergens: ["Soy", "Fish", "Sesame", "Wheat"],
  dishes: [
    Dish(
      name: "Salmon Teriyaki Onigiri",
      description: "Rice ball with teriyaki salmon",
      price: 6.00,
      allergens: ["Soy", "Fish"],
      imagePath: "assets/food/salmon_onigiri.jpg",
      isVegan: false,
    ),
    Dish(
      name: "Kombu Tsukudani Onigiri",
      description: "Rice ball with seaweed",
      price: 5.50,
      allergens: ["Soy", "Sesame"],
      imagePath: "assets/food/kombu_onigiri.jpg",
      isVegan: true,
    ),
    Dish(
      name: "Soba Noodle Salad",
      description: "Cold soba noodle salad",
      price: 8.00,
      allergens: ["Wheat", "Soy", "Sesame"],
      imagePath: "assets/food/soba_salad.jpg",
      isVegan: true,
    ),
    Dish(
      name: "Ube Mochi",
      description: "Purple yam mochi",
      price: 5.00,
      allergens: [],
      imagePath: "assets/food/ube_mochi.jpg",
      isVegan: true,
    ),
    Dish(
      name: "Pina Colada Amazake",
      description: "Sweet fermented rice drink",
      price: 5.50,
      allergens: [],
      imagePath: "assets/food/amazake.jpg",
      isVegan: true,
    ),
    Dish(
      name: "Sparkling Tea",
      description: "Refreshing sparkling tea",
      price: 4.50,
      allergens: [],
      imagePath: "assets/food/sparkling_tea.jpg",
      isVegan: true,
    ),
  ],
),
];