...
Future<void> _fetchDishesData() async {
    ...
    try {
      ...
      if (response == null || response.isEmpty) { // Added null check for safety
        setState(() {
          _dishes = [];
          _isLoading = false;
        });
      ...
      final List<Map<String, dynamic>> dishDataList = List<Map<String, dynamic>>.from(response);
      ...
      setState(() {
        _dishes = dishDataList.map<Dish>((data) {
          ...
          final List<String> allergensList = allergenString
                  ?.split(',')
                  .map((a) => a.trim())
          ...
          return Dish(
            ...
            imagePath: data['imagePath'] as String? ?? 'assets/default_dish.png',
  ...
  Widget build(BuildContext context) {
    ...
    return Stack(
      children: [
        ...
        Center(
          child: Container(
            width: MediaQuery.of(context).size.width * 0.85, // Slightly wider for better content fit
            height: MediaQuery.of(context).size.height * 0.75, // Slightly taller
            constraints: BoxConstraints(
              maxHeight: MediaQuery.of(context).size.height * 0.9, // Adjusted max height
              maxWidth: 600, // Added max width for larger screens/tablets
            ),
            margin: const EdgeInsets.symmetric(horizontal: 20, vertical: 20), // Adjusted margin
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        image: DecorationImage(
           ...
           image: _getBoothImage(imagePath ?? 'assets/default_booth.png'),
          fit: BoxFit.cover,
            ...
            children: [
              Text(
                ...
                style: const TextStyle(
                  ...
                  shadows: [
                    Shadow(blurRadius: 4, color: Colors.black87), // Enhanced shadow
                  ],
                ),
                maxLines: 2,
              ...
              Text(
                ...
                style: const TextStyle(
                  ...
                  shadows: [
                     Shadow(blurRadius: 3, color: Colors.black54), // Enhanced shadow
                  ],
                ),
                 maxLines: 1,
                overflow: TextOverflow.ellipsis,
  ...
  Widget _buildBoothContent() {
     // ... (rest of _buildBoothContent remains the same)
      ...
      padding: const EdgeInsets.only(top: 16, bottom: 16, left: 8, right: 8), // Adjust padding
      child: Column(
        children: [
          _buildSection(
            "Payment Options", // More descriptive title
            _buildPaymentOptions(widget.booth.payments ?? []), // Handle null payments list
          ),
          const SizedBox(height: 20), // Increased spacing
          _buildSection(
            "Vegan Friendly", // More descriptive title
            _buildVeganism(widget.booth.isVegan ?? false), // Handle null isVegan
          ),
          const SizedBox(height: 20), // Increased spacing
           if (_dishes.isNotEmpty || _isLoading) // Only show dish section if loading or has dishes
             _buildSection(
              "Menu", // Simpler title
              _buildDishesSection(
                _dishes,
           ...
           if (!_isLoading && _dishes.isEmpty && _errorMessage == null)
             const Padding( // Show message if no dishes and not loading/error
                padding: EdgeInsets.symmetric(vertical: 30.0),
                child: Text("No menu items available for this booth yet.", textAlign: TextAlign.center),
             ),
  ...
  Widget _buildSection(String title, Widget content) {
    // ... (rest of _buildSection remains the same)
      ...
      children: [
        Padding(
          padding: const EdgeInsets.symmetric(horizontal: 16.0), // Padding for title
          child: Text(
            title,
            style: const TextStyle(
              fontSize: 18,
        ...
        Padding(
          padding: const EdgeInsets.symmetric(horizontal: 8.0), // Padding for content
          child: content,
        ),
        const Divider(height: 30, thickness: 1, indent: 20, endIndent: 20), // Add divider between sections
      ],
    );
  }
  ...
  Widget _buildDishesSection(
    ...
    // ... (rest of _buildDishesSection remains the same)
      final dishesToShow = selectedAllergens.isEmpty
        ? dishes
        : dishes.where((dish) {
            // Only show dishes that don't contain any of the selected allergens
            final dishAllergensLower = dish.allergens.map((a) => a.toLowerCase()).toSet();
            final selectedAllergensLower = selectedAllergens.map((a) => a.toLowerCase()).toSet();
            return dishAllergensLower.intersection(selectedAllergensLower).isEmpty;
          }).toList();

    if (dishesToShow.isEmpty && dishes.isNotEmpty) { // Adjusted condition
      return const Center(
        child: Padding(
     ...
     if (dishesToShow.isEmpty && dishes.isEmpty) { // Should be covered by outer checks, but for safety
    ...
    return Column(
      children: dishesToShow.map((dish) {
         return Padding(
           padding: const EdgeInsets.symmetric(vertical: 4.0), // Spacing between cards
           child: DishCard(
             // Use unique key based on dish ID
             key: ValueKey('dish_${dish.id}'), // Ensure dish.id is unique and suitable as a key
             dish: dish,
      ...
      child: Wrap(
        ...
        children: [
          ...
          _buildPaymentItem(
            ...
            acceptedPaymentsLower.contains("credit card") || acceptedPaymentsLower.contains("credit"),
          ),
          _buildPaymentItem(
            "PayPal", // Label for UI
  ...
  Widget _buildPaymentItem(String label, String assetPath, bool isAccepted) {
    // ... (rest of _buildPaymentItem remains the same)
      return Opacity( // Fade out if not accepted
        ...
        children: [
          Container(
            ...
            decoration: BoxDecoration(
              ...
              boxShadow: isAccepted ? [ // Only shadow if accepted
                BoxShadow(
   ...
   Widget _buildVeganism(bool isVegan) {
     // ... (rest of _buildVeganism remains the same)
        return Column( // Removed Center as _buildSection handles alignment
       mainAxisSize: MainAxisSize.min,
       children: [
         Container(
           width: 45,
           ...
           child: Image.asset(
             "assets/vegan.png", // Ensure this asset exists
              ...
              color: isVegan ? Colors.green.shade600 : Colors.grey.withOpacity(0.6),
         ...
         Text(
           isVegan ? "Offers Vegan Options" : "No Specific Vegan Options", // More descriptive text
           style: TextStyle(
             fontSize: 13,
             fontWeight: FontWeight.normal,
...
class DishCard extends StatefulWidget {
  final Dish dish; // Expects dish.allergens to be List<String> correctly populated
  final List<String> selectedAllergens;
...
class _DishCardState extends State<DishCard> {
  ...
  final String _defaultAllergenIcon = "assets/allergens/default.png"; // Default icon asset path
  ...
  Widget build(BuildContext context) {
    ...
    return Card(
      ...
      child: Column(
        ...
        children: [
          Stack( // Image, Badges, Name/Price Overlay
            children: [
              _buildDishImage(widget.dish.imagePath ?? 'assets/default_dish.png'),
              Positioned( // Badges
                top: 8, left: 8, right: 8,
                child: Row(
                  mainAxisAlignment: MainAxisAlignment.spaceBetween,
                  children: [
                    if (containsSelectedAllergens) _buildBadge("⚠ Contains Selected Allergen", Colors.red.withOpacity(0.9), Icons.warning_amber_rounded),
                    const Spacer(),
                    if (widget.dish.isVegan == true) _buildBadge("Vegan", Colors.green.withOpacity(0.85), Icons.eco),
                  ],
                ),
              ),
              Positioned( // Name/Price Overlay
                ...
                child: Container(
                  ...
                  child: Row(
                    ...
                    children: [
                      Expanded(child: Text(widget.dish.name, style: const TextStyle(color: Colors.white, fontSize: 17, fontWeight: FontWeight.bold), maxLines: 1, overflow: TextOverflow.ellipsis)),
                      const SizedBox(width: 8),
                      Text(widget.dish.price > 0 ? '\$${widget.dish.price.toStringAsFixed(2)}' : 'TBD', style: const TextStyle(color: Colors.white, fontSize: 16, fontWeight: FontWeight.w600)),
                    ],
                  ),
                ),
              ),
          ...
          Padding( // Description & Allergens
            padding: const EdgeInsets.all(12),
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                if (widget.dish.description.isNotEmpty) Padding(padding: const EdgeInsets.only(bottom: 12.0), child: Text(widget.dish.description, style: TextStyle(fontSize: 14, color: Colors.grey[700], height: 1.4))),
  ...
  Widget _buildBadge(String text, Color color, IconData icon) {
     // ... (This helper remains the same) ...
       ...
       decoration: BoxDecoration(
         ...
         boxShadow: [ // Add subtle shadow to badges
            BoxShadow(
           ...
           child: InkWell(
             ...
             child: Padding(
               ...
               child: Row(
                 mainAxisAlignment: MainAxisAlignment.spaceBetween,
                 children: [
                   Text("Contains Allergens (${widget.dish.allergens.length})", style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w600, color: Color(0xFF7A4E2C))),
                   Icon(_showAllergenDetails ? Icons.keyboard_arrow_up : Icons.keyboard_arrow_down, size: 20, color: const Color.fromARGB(255, 151, 73, 0)),
                 ],
               ),
             ),
           ),
         ),
         AnimatedSize( // Animated dropdown content
           duration: const Duration(milliseconds: 250),
           curve: Curves.easeInOut,
           child: Container(
             constraints: BoxConstraints(maxHeight: _showAllergenDetails ? 200 : 0), // Adjust max height if needed
             width: double.infinity,
             child: _showAllergenDetails
  ...
  Widget _buildAllergenDetailsContent() {
    ...
    return Padding(
      ...
      child: Wrap(
        ...
        children: widget.dish.allergens.map((allergenName) {
          // Find the corresponding local icon path
          String localIconPath = _localBackupIcons[allergenName] ?? _localBackupIcons[allergenName.split(' ').first] ?? _defaultAllergenIcon;

          // Check if this allergen is among the user's selected ones
          bool isSelected = widget.selectedAllergens.any((sel) => sel.toLowerCase() == allergenName.toLowerCase());
              ...
              child: Column(
                ...
                children: [
                  Container( // Icon background
                    ...
                    child: Center(
                      child: Image.asset(
                        ...
                        width: 28, height: 28,
                        fit: BoxFit.contain,
                        color: const Color.fromARGB(255, 107, 53, 1), // Apply color tint
                        colorBlendMode: BlendMode.srcIn,
                        errorBuilder: (context, error, stackTrace) {
                          // Fallback if even the local asset fails
                          print("!!! Error loading local allergen asset: $localIconPath");
                          return Icon(Icons.warning_amber_rounded, color: const Color.fromARGB(255, 107, 53, 1), size: 24);
                        },
                      ),
                    ),
                  ),
                  ...
                  Text( // Allergen name text
                    ...
                    style: TextStyle(
                      fontSize: 12,
                      color: isSelected ? Colors.red.shade800 : const Color.fromARGB(255, 107, 53, 1),
  ...
  Widget _buildDishImage(String imagePath) {
    // ... (This helper remains the same) ...
       ...
       child: const Center(
         child: Icon(Icons.restaurant_menu, size: 50, color: Colors.grey), // Different icon
       ),
     );
     ...
     if (imagePath.startsWith('http')) {
       return Image.network(
         ...
         errorBuilder: (context, error, stackTrace) => placeholder, // Show placeholder on error
       );
     } else {
       // Assume asset path
       return Image.asset(
